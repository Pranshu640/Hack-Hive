...
const TeamFormation = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/team-requests/received`, {
  ...
  const fetchTeamDetails = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/teams/details`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/v1/team-requests/search?query=${encodeURIComponent(searchQuery)}`, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const sendRequest = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/team-requests/send`, 
        { receiverId: userId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleRequest = async (requestId, action) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/team-requests/handle`,
        { requestId, action },
        { headers: { Authorization: `Bearer ${token}` } }
      );
